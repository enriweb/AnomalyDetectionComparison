...
Metrics recorded per run:
    ...
    Efficiency — n_params, FLOPs (encoder, thop optional),
                 inference time GPU + CPU (DINOv2 encoder),
...
# ── Startup banner ────────────────────────────────────────────
print("=" * 60)
print(f"  Dinomaly  |  Encoder: {ENCODER}")
print(f"  dropout=0.2  |  decoder_depth=8  |  2 groups")
            ...
            try:
                ...
                if HAS_THOP:
                    try:
                        ...
                        # DINOv2 ViT-B/14 uses 224×224 for a 14-patch-size model
                ...
                # ── CPU inference: encoder on 1 image ─────────────────
                inference_cpu_s = None
                peak_cpu_mb = None
                mean_cpu_mb = None
                try:
                    import tracemalloc
                    print(f"  → Timing CPU inference (encoder, 1 image × 5 runs)...")
                    _feat = model.model.encoder.cpu().eval()
                    _dev = next(_feat.parameters()).device
                    if str(_dev) != "cpu":
                        raise RuntimeError(f"Encoder still on {_dev} after .cpu()")
