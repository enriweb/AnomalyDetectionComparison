...
Metrics recorded per run:
    ...
    Efficiency — n_params (CNN + ViT combined), FLOPs (CNN branch, thop optional),
                 inference time GPU + CPU (WRN50 CNN branch),
    ...
    for cat_idx, category in enumerate(categories, 1):
        ...
        for run in range(runs_done, N_RUNS):
            ...
            try:
                ...
                # ── FLOPs: CNN backbone forward on one image ──────────
                flops_M = None
                if HAS_THOP:
                    try:
                ...
                # ── CPU inference: CNN branch on 1 image ──────────────
                # Times WRN50 CNN branch only; ViT branch timing would be separate.
                inference_cpu_s = None
                peak_cpu_mb = None
                mean_cpu_mb = None
                try:
                    import tracemalloc
                    print(f"  → Timing CPU inference (CNN branch, 1 image × 5 runs)...")
                    _feat = model.model.feature_extractor.cpu().eval()
                    _dev = next(_feat.parameters()).device
                    if str(_dev) != "cpu":
                        raise RuntimeError(f"CNN backbone still on {_dev} after .cpu()")
                    ...
                    print(f"  → CPU inference (CNN): {inference_cpu_s:.4f}s (avg 5 runs)"
