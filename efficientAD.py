...
Metrics recorded per run:
    ...
    Efficiency — n_params, FLOPs (student backbone, thop optional),
                 inference time GPU + CPU (PDN student),
...
# ── Startup banner ────────────────────────────────────────────
print("=" * 60)
print(f"  EfficientAD-{VARIANT.upper()}  |  PDN teacher_out=384  |  max_steps=70000")
...
for ds_idx, (ds_name, DataModule, categories, root) in enumerate(DATASETS, 1):
    ...
    for cat_idx, category in enumerate(categories, 1):
        ...
        for run in range(runs_done, N_RUNS):
            ...
            image_f1max = image_fnr = pixel_fnr = test_results = metrics = None
            try:
                ...
                test_metrics = [image_auroc, pixel_pro]
                if HAS_F1MAX:
                    image_f1max = F1Max(fields=["pred_score", "gt_label"], prefix="image_")
                    test_metrics.append(image_f1max)
                if HAS_FNR:
                ...
                engine = Engine(max_steps=70000, logger=False, callbacks=[CompactBar()])
                engine.fit(model=model, datamodule=datamodule)
                print(f"  → Training complete.")

                # ── FLOPs: PDN student forward on one image ───────────
                flops_M = None
                if HAS_THOP:
                    try:
                        ...
                        with warnings.catch_warnings():
                            ...
                            _flops, _ = _thop_profile(
                                model.model.student, inputs=(_dummy,), verbose=False
                            )
                ...
                # ── CPU inference: PDN student on 1 image ────────────
                inference_cpu_s = None
                peak_cpu_mb = None
                mean_cpu_mb = None
                try:
                    import tracemalloc
                    print(f"  → Timing CPU inference (PDN student, 1 image × 5 runs)...")
                    _feat = model.model.student.cpu().eval()
                    _dev = next(_feat.parameters()).device
                    if str(_dev) != "cpu":
                        raise RuntimeError(f"Student still on {_dev} after .cpu()")
                    _dummy_cpu = torch.randn(1, 3, 256, 256)
                    for _ in range(3):
                        with torch.no_grad():
                            _feat(_dummy_cpu)
                ...
                del image_auroc, pixel_pro, image_f1max, image_fnr, pixel_fnr
...
lines.append("  PDN teacher_out=384  |  image_size=256×256  |  max_steps=70000")
lines.append(SEP)
...
for ds_name, _, categories, _ in DATASETS:
    ...
    # ── Accuracy table ────────────────────────────────────────
    lines.append(f"\n--- {label} — Accuracy ---")
    HDR_A = (f"{'Category':<15} {'Img AUROC':>{C}} {'AUPRO':>{C}} {'Img F1Max':>{C}}"
    ...
    for cat in categories:
        ...
        r = {k: (_mean(runs, k), _std(runs, k)) for k in
             ("image_AUROC", "pixel_AUPRO", "image_F1Max", "image_FNR", "pixel_FNR")}
    ...
    if cat_acc:
        avg = {k: statistics.mean(v for v in (c[k] for c in cat_acc.values()) if v == v)
               for k in ("image_AUROC", "pixel_AUPRO", "image_F1Max", "image_FNR", "pixel_FNR")}
        lines.append("-" * len(HDR_A))
        lines.append(
            f"{'MEAN':<15}"
...
if n_all:
    ...
    lines.append(f"{'='*80}")
    lines.append(f"  Img AUROC : {oi:.2f}%   AUPRO : {opr:.2f}%   Img F1Max : {of1i:.2f}%")
