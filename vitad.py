...
Usage:
    python vitad.py          # 1 run (default)
    python vitad.py 3        # 3 runs, results averaged
...
Progress is checkpointed to results/vitad_progress.json after every
...
# VitAd model: graceful fallback for anomalib versions that do not include it.
try:
    from anomalib.models import VitAd
    HAS_VITAD = True
except ImportError:
    HAS_VITAD = False
    VitAd = None
...
PROGRESS_FILE = "results/vitad_progress.json"

# -- Startup banner -----------------------------------------------------------
print("=" * 60)
print("  ViTAD  |  Vision Transformer AD baseline")
print(f"  Runs per category : {N_RUNS}")
print(
    f"  Categories total  : {total_categories}  "
    f"({len(MVTEC_CATEGORIES)} MVTec + {len(VISA_CATEGORIES)} VisA)"
)
...
print(
    f"  F1Max metric      : "
    f"{'available' if HAS_F1MAX else 'not in this anomalib version - skipped'}"
)
if not HAS_VITAD:
    raise ImportError("VitAd not available in this anomalib version. Install/update anomalib with VitAd support.")
                ...
                if HAS_THOP:
                    try:
                        ...
                        with warnings.catch_warnings():
                            ...
                            _flops, _ = _thop_profile(model.model.backbone, inputs=(_dummy,), verbose=False)
...
out_path = "results/vitad_combined.txt"
...
lines.append(SEP)
lines.append(f"  ViTAD Results  (N={N_RUNS} run{'s' if N_RUNS > 1 else ''})")
lines.append("  Backbone: ViT (VitAd defaults)")
lines.append(SEP)
