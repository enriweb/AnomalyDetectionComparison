...
PROGRESS_FILE = "results/anomalydino_progress.json"
...
def _fmt(mean: float, std: float, decimals: int = 1) -> str:
    ...
    fmt = f"{{:{5+decimals}.{decimals}f}}+_{{:4.{decimals}f}}" if N_RUNS > 1 else f"{{:10.{decimals}f}}"
    return fmt.format(mean, std) if N_RUNS > 1 else fmt.format(mean)

# - Final results tables ------------------------------------------------------
out_path = "results/anomalydino_combined.txt"
...
lines.append(SEP)
lines.append(f"  AnomalyDINO Results  (N={N_RUNS} run{'s' if N_RUNS > 1 else ''})")
lines.append(f"  Encoder: {ENCODER}  |  Neighbours: {NUM_NEIGHBOURS}  |  Coreset: {SAMPLING_RATIO}  |  Masking: {MASKING}")
lines.append(SEP)
...
for ds_name, _, categories, _ in DATASETS:
    ...
    for cat in categories:
        ...
        cat_eff[cat] = {"np": np_, "gpu": gi_, "pgm": pgm_, "mgm": mgm_}
        lines.append(
            f"{cat:<15}"
            f" {_fe(np_,  '{:,.0f}')}"
            f" {_fe(gi_,  '{:.3f}')}"
