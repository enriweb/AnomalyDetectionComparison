...
Metrics recorded per run:
    ...
    Efficiency — n_params, FLOPs (student backbone, thop optional),
                 inference time GPU + CPU (WRN50 student),
                 peak + mean GPU MB, peak + mean CPU MB

Progress is checkpointed to results/uninet_progress.json after every
...
# ── Startup banner ────────────────────────────────────────────
print("=" * 60)
print("  UniNet  |  Backbone: WRN50  |  λ=0.7  |  T=2  |  α=0.01  |  β=0.03")
    ...
    for cat_idx, category in enumerate(categories, 1):
        ...
        for run in range(runs_done, N_RUNS):
            ...
            try:
                ...
                print(f"  → Building model (WRN50 student+teacher)...")
                # (§4, §5.1)
                model = UniNet(
                ...
                # ── CPU inference: student backbone on 1 image ────────
                inference_cpu_s = None
                peak_cpu_mb = None
                mean_cpu_mb = None
                try:
                    import tracemalloc
                    print(f"  → Timing CPU inference (student, 1 image × 5 runs)...")
