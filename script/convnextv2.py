"""
PatchCore with ConvNeXt V2-Tiny backbone

Usage:
    python convnextv2.py          # 1 run (default)
    python convnextv2.py 3        # 3 runs, results averaged

Backbone  : convnextv2_tiny.fcmae_ft_in22k_in1k  (ImageNet-22k → 1k fine-tune)
Layers    : stages.2 + stages.3  (384-d @ 14×14, 768-d @ 7×7)
Coreset   : 1%  (matches PatchCore-1% baseline for fair CNN backbone comparison)

Metrics recorded per run:
    Accuracy  — image AUROC, pixel AUROC, pixel AUPRO,
                image F1Max (optimal threshold), pixel F1Max
    Efficiency — n_params, FLOPs (backbone, FlopCounterMode), peak GPU MB,
                 deploy latency (raw frame -> anomaly decision, batch=1):
                 total mean/p95/p99 + per-stage pre/fwd/post breakdown

Progress checkpointed to results/convnextv2_results.csv after every run.
Auto-resumes from CSV on restart.
"""
...
csv_path    = "results/convnextv2_results.csv"
combined_txt = "results/convnextv2_combined.txt"
CKPT_DIR    = "results/Patchcore"

print("=" * 60)
print(f"  ConvNeXt V2-Tiny  |  PatchCore-1%  |  stages.2+3")
    ...
    for ds_idx, (ds_name, DataModule, categories, root) in enumerate(DATASETS, 1):
        ...
        for cat_idx, category in enumerate(categories, 1):
            ...
            try:
                ...
                print(f"  → Building model (ConvNeXt V2-Tiny backbone)...")
                model = Patchcore(
                    backbone=BACKBONE,
                    layers=LAYERS,
                    pre_trained=True,
...
lines = []
lines.append("=" * 140)
lines.append(f"  ConvNeXt V2-Tiny + PatchCore-1%  (N={N_RUNS}, backbone={BACKBONE}, layers={LAYERS}, coreset={CORESET})")
