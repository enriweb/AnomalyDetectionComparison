...
Metrics recorded per run:
    ...
    Efficiency — n_params, FLOPs (backbone, thop),
                 inference time GPU, peak/mean GPU MB,
                 gpu_throughput (img/s, batched), cpu_latency_ms

Progress checkpointed to results/fastvit_patchcore_progress.json.
"""
    ...
    for cat_idx, category in enumerate(categories, 1):
        ...
        for run in range(runs_done, N_RUNS):
            ...
            if not vram_ok():
                ...
                free_gpu()
                if not vram_ok():
                    print(f"  [SKIP] VRAM still over threshold. Skipping run {run+1}.")
                    continue
...
out_path = "results/fastvit_patchcore_combined.txt"
...
SEP = "=" * 80
lines.append(SEP)
lines.append(f"  FastViT-PatchCore Results  (N={N_RUNS} run{'s' if N_RUNS > 1 else ''})")
lines.append(f"  Backbone: {BACKBONE}  |  Layers: {LAYERS}  |  Coreset: {int(CORESET_RATIO*100)}%")
lines.append(SEP)
...
if n_all:
    ...
    ogt  = _omean("gpu_throughput")
    ocl  = _omean("cpu_latency_ms")
    lines.append(f"\n{'='*80}")
    ...
    lines.append(f"  GPU inf   : {ogi:.3f}s   GPU thru  : {ogt:.1f} img/s   CPU lat : {ocl:.1f} ms")
